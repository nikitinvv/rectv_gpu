...
def reconstruct(h5fname, sino, nframes, frame, nproj, binning, tv):
    ...
    if tv:
        ...
        # Make a class for tv
        cl = rectv.rectv(N, Ntheta, M, nframes, Nz,
                         Nzp, ngpus, lambda0, lambda1)
        # Run iterations
        ...
        for time_frame in range(0, nframes):
            rec0 = tomopy.recon(data[time_frame*nproj:(time_frame+1)*nproj], theta[time_frame*nproj:(
                time_frame+1)*nproj], center=rot_center-np.mod(time_frame, 2), algorithm='gridrec')
...
def main(arg):
    ...
    parser.add_argument("--binning", nargs='?', type=str,
                        default=0, help="binning projections: 0 (default 0)")
    parser.add_argument("--frame", nargs='?', type=str, default=92,
